...
\begin{enumerate}
  ...
  Draw arrows going from the less plausible worlds to more plausible worlds 
	...
	\item Assume that the controller is known to be \textit{infallible}, and so 
	that the robot performs an \textit{update} with the announced sentence. 
	\textit{Represent the robot's new belief structure (as a plausibility model) 
	after this update}. 
	...
	\item What would have been the final belief structure (plausibility	model) of 
	the robot if the above three events happened in \textit{a different order}, 
...
There are two agents, Alice and Bob, and \textit{it is common knowledge that 
neither of them can see the coin, but that for some reason they both believe 
that the coin lies Tails up}.

\begin{enumerate}
	\item \textit{Draw a multi-agent plausibility model} \textbf{M$_0$} (with two 
	agents, $a$ for Alice and $b$ for Bob, and atomic sentence $H$ and $T$) to 
	accurately represent all the information above.
	...
	\item Some external referee publicly announces: ``\textit{The coin lies Heads 
	up}''. 
	It is \textit{common knowledge that: Bob strongly trusts the referee, but 
	that Alice is neutral towards the referee} (neither trusts nor distrusts him).
	
	\textit{Represent} this action an an \textbf{event plausibility model} 
	\boldmath{$\Sigma$}.
	...
	\item \textit{Represent (draw) a model} \textbf{M$_1$} for the situation 
	\textit{after} the action described in the previous part, by computing the 
	Action-Priority update \textbf{M}$_1$ = \textbf{M$_0$} $\otimes$ 
	\boldmath{$\Sigma$}.
	...
	\item We now change the scenario as follows. 
	It is \textit{still common knowledge that Bob strongly trusts the referee}, 
	but Alice's attitude is NOT common knowledge. 
	Instead, it is common knowledge that: \textit{either Alice strongly trusts 
	the referee, or she is neutral (neither trusting nor distrusting), or she 
	strongly distrusts him}. 
	Moreover, it is also common knowledge that: \textit{Bob believes that Alice 
	trusts the referee}; but that, \textit{IF given that this belief is wrong, 
	he'd believe that she's neutral} (rather than that she is strongly 
	distrusting the referee).
	
	Of course, we also assume \textit{each agent knows his/her own attitude}.
	
	As before, the referee publicly announces: 
	``\textit{The coin lies Heads up}''. 
	
	\textit{Represent} this action as an \textbf{event plausibility model} 
	\boldmath{$\Sigma'$}.
	...
	\item Starting from the original situation (in part 1), suppose the	action 
	that we described in the previous part happens. 
	
	\textit{Represent (draw) a model} \textbf{M$_1'$} for the situation 
	\textit{after} this action, by computing the Action-Priority update 
	\textbf{M$'_1$} = \textbf{M$_0$} $\otimes$ \boldmath{$\Sigma'$}.
